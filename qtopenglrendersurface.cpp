...
QTOpenGLRenderSurface::QTOpenGLRenderSurface(QScreen *outputScreen,
                                             QObject *parent,
...
QTOpenGLRenderSurface::~QTOpenGLRenderSurface()
{

}

const QSurfaceFormat &QTOpenGLRenderSurface::getOpenGLFormat()
{
    return openGLFormat;
}

QOpenGLContext *QTOpenGLRenderSurface::getOpenGLContext()
{
    return openGLContext;
}

void QTOpenGLRenderSurface::setFrameRate(float fps)
{
    QTOpenGLRenderer::setFrameRate(fps);
}

void QTOpenGLRenderSurface::start()
{
    float timeOut = 1000.0f/renderSpecs.frameRate;
    syncTimer->start(timeOut);
}

void QTOpenGLRenderSurface::stop()
{
    syncTimer->stop();
}

void QTOpenGLRenderSurface::renderFrame()
{
    updateStartTime();
...
void QTOpenGLRenderSurface::initializeFBO()
{
...
void QTOpenGLRenderSurface::initializeShaderProgram()
{
...
void QTOpenGLRenderSurface::initializeVertexBuffers()
{
...
void QTOpenGLRenderSurface::initializeUniforms()
{

}

void QTOpenGLRenderSurface::updateUniforms()
{
    shader->bind();
...
void QTOpenGLRenderSurface::initializeTimer()
{
    syncTimer = new QTimer(this);
    syncTimer->setTimerType(Qt::PreciseTimer);

    QObject::connect(syncTimer,&QTimer::timeout,this,&QTOpenGLRenderSurface::renderFrame);
}

void QTOpenGLRenderSurface::swapSurfaceBuffers()
{
    openGLContext->swapBuffers(this);
}

bool QTOpenGLRenderSurface::makeContextCurrent()
{
    return openGLContext->makeCurrent(this);
}

void QTOpenGLRenderSurface::doneContextCurrent()
{
